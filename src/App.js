...
import { authByEmailLink, checkIfSignInByLink } from "./domain/auth";
...
class App extends React.Component {
  ...
  loginWithEmail() {
    authByEmailLink(this.state.email);
  }
  ...
  async setData(searchQuery) {
    ...
    if (Date.now() > this.state.ExpiryDate) {
      ...
      console.log(
        "this has an access token" + this.state.AccessToken.substring(10, 25)
      );
    }
    ...
    const ebayresponse = await fetch(
      `https://loma-cors.herokuapp.com/https://api.ebay.com/buy/browse/v1/item_summary/search?q=${searchQuery}&limit=200&filter=conditions:{UNSPECIFIED|USED},itemLocationCountry:GB&fieldgroups=EXTENDED`,
      ebayrequestOptions
    );
    ...
    if (this.state.depopData.status === "unresolved") {
      ...
      const depopDataFiltered = depopData.filter((listing) => {
        if (
          isNull(listing.title).toLowerCase().includes(this.state.searchTerm)
        ) {
          return true;
        } else if (
    ...
    if (this.state.gumtreeData.status === "unresolved") {
      ...
      const gumtreeResponse = await fetch(gumtree_url);
      const gumtreeData = await gumtreeResponse.json();
      const gumtreeDataFiltered = gumtreeData.filter(
        (listing) =>
    ...
    const etsyRaw = await fetch(
      `https://loma-cors.herokuapp.com/https://openapi.etsy.com/v2/listings/active?keywords=${searchQuery}&tags=vintage&limit=200&region=GB&api_key=sh1n3isgj6bax7eotlor7m4s&includes=Images`
    );
    ...
    this.state.resultsDisplay === "⚡️ Trending ⚡️"
      ? this.setState((prevState) => ({
      ...
      : this.state.resultsDisplay === "Lowest price first ▼"
      ? this.setState((prevState) => ({
